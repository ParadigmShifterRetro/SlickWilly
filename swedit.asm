...
.nexttileentry
	...
	; copy crumbly tile data into crumbly tiles area
	ld de, tile_dist_table+TILEDISTTBL.crumblyidx
	ld a, (de)
	or a
	jr z, .nocrumblies ; no crumbly tiles this level
	ld c, a
...
.findnextidx
	...
	ld (crumbly_end_idx), a

	sub c
	ld b, a ; total number of crumbly tiles this level

	push hl

	ld de, crumbly_tiles_area+7

	ld hl, level_tiles
	ld a, c
	rlca
	add l
	ld l, a ; HL should point to first crumbly tile
...
draw_level:
	...
	ld de, 0
	ld a, (tile_dist_table+TILEDISTTBL.crumblyidx)
	ld d, a
	or a
	jr z, .nextcollcell ; no crumblies on this level
	ld a, (crumbly_end_idx)
	ld e, a
...
crumbly_tiles_area BLOCK 15*MAXCRUMBLIES

        IF   (crumbly_tiles_area/256)-($/256)
        ERROR "crumbly_tiles_area must be within single 256 byte block"
        ENDIF

crumbly_end_idx	db 0
